...
/*
 HTTP protocol is connection-less and state-less. Stateless means when session's state or request of the user is finished or done then site does not have any info about the user and in the next request, it would be new from scratch. That's why cookies and caches concept came in, using which request could know about user before hand. and hence help the server to know about user very well.
 */
...
print(URLSessionConfiguration.default.requestCachePolicy)
...
//: to check the status code: 200 means OK:
    ...
    /* Session Task is created in suspended state, so you should definitely resume it. */
    task.resume()
  }
