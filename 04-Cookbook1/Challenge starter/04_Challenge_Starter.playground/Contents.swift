...
class QueryService {
    ...
    func updatePosts(with data: Data) {
        let decoder = JSONDecoder()
        do {
            posts = try decoder.decode([Post].self, from: data)
        } catch let decodingError as NSError {
            errorMessage += "Error while decoding data : \(decodingError.localizedDescription)"
        }
...
extension QueryService {
    ...
    func getPostsFromServer() {
        defaultSession.dataTask(with: url) { (data, response, error) in
            /* error means client side error but response error means server side error. */
            
            if let error = error {
                self.errorMessage += "Error during dataTask : \(error.localizedDescription)"
            } else if let data = data, let response = response as? HTTPURLResponse, response.statusCode == 200 {
                self.updatePosts(with: data)
                self.errorMessage
                self.posts
            }
        }.resume()
...
/* Already did above */
QueryService().getPostsFromServer()
