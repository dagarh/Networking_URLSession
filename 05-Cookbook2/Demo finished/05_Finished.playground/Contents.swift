...
/* There is a request associated with every task. So one request per task, not per session. */

/* It has not run the task till now because we did not call ".resume()". It is in suspended state. We just want to check the request properties. */
let task = session.dataTask(with: url)


/* All the below properties of a "URLRequest struct" are read-only. You can not modify these here. In order to modify these you need to create separate URLRequest object.  */
task.currentRequest?.httpMethod
task.currentRequest?.url
task.currentRequest?.description

task.currentRequest?.allHTTPHeaderFields // type of this is [String:String]?

task.currentRequest?.httpBody // type of this is Data?

task.currentRequest?.allowsCellularAccess // = false  // error: currentRequest is read-only
task.currentRequest?.httpShouldHandleCookies // yes in memory because it is ephemeral(private) session.
task.currentRequest?.timeoutInterval // obviously this is timeoutInterval for request, not for resource.
/* There is one more property i.e multipath network type. So don't get confused between two. */
...
let postTask = session.dataTask(with: request) { data, response, error in
    ...
    /* In HTTPURLResponse's header, "Content-Length" is slightly higher than the "Content-Length" of URLRequest's header. Because reponse body also contains "id" now. */
    response.allHeaderFields
    
    /* data(of type Data?) is http body of response. */
